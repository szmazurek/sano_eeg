...
def run_preprocessing(
    ...
    if apply_ica and apply_pca:
        raise ValueError("Values of apply_pca and apply_ica cannot both be True! Choose only one method of artifact removal.")
...
def preprocess_dataset(subjects_with_seizures_path, dataset_dirpath, preprocessed_dirpath):
    ...
    """
    subjects_with_seizures = [subject[:-1] for subject in open(subjects_with_seizures_path,'r').readlines()]
...
def prepare_timestep_array(array, timestep, overlap):
    ...
    features = [
        array[:, :, i : i + timestep]
        for i in range(0, array.shape[2] - timestep, timestep - overlap)
    ]
...
def extract_training_data_and_labels(
    ...
    for n, start_ev in enumerate(start_ev_array):
        fs = raw.info["sfreq"]
        seizure_lookback = fs * seizure_lookback
        ...
        if prev_event_time > seizure_lookback:
            interictal_period = raw.get_data(
                start=start_ev - seizure_lookback, stop=start_ev
            )
        else:
            interictal_period = raw.get_data(
                start=start_ev - prev_event_time, stop=start_ev
            )
        ...
        seizure_period = raw.get_data(start=start_ev, stop=stop_ev_array[n])
        seizure_period = (
        ...
        seizure_event_labels = np.ones(
            seizure_features.shape[0]
        ...
        # np.save()
    return interictal_features, interictal_event_labels, interictal_event_time_labels
